...
export const NESTO_SIGNUP_AGENT_USER_COOKIE = "nesto_signup_agent_user_id";
export const NESTO_SIGNUP_AGENCY_COOKIE = "nesto_signup_agency_id";
...
export async function getServerSessionAgencyId() {
  ...
  return cookieStore.get(NESTO_AGENCY_COOKIE)?.value
    ?? cookieStore.get(NESTO_SIGNUP_AGENCY_COOKIE)?.value
    ?? cookieStore.get(DEMO_AGENCY_COOKIE)?.value
    ?? null;
}

export async function getServerSessionAgentUserId() {
  const cookieStore = await cookies();
  return cookieStore.get(NESTO_AGENT_USER_COOKIE)?.value
    ?? cookieStore.get(NESTO_SIGNUP_AGENT_USER_COOKIE)?.value
    ?? null;
}
